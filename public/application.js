...
var signInSuccess = function signInSuccess(data) {
  ...
  api.getFiles().done(getFilesSuccess).fail(getFilesFailure);
};
...
var uploadFailure = function uploadFailure(error) {
  console.error(error);
};
// get files

var getFilesSuccess = function getFilesSuccess(data) {
  console.table(data);
  logic.loopFiles(data);
  $('#myFile-data').removeClass('hidden');
  $('#theirFile-data').removeClass('hidden');
};
var getFilesFailure = function getFilesFailure(error) {
  console.error(error);
};
...
module.exports = {
  ...
  getFilesFailure: getFilesFailure,
  oldUser: oldUser,
  newUser: newUser,
...
var config = {
  apiOrigins: {
    production: 'https://powerful-river-39157.herokuapp.com'
  }
};
...
var onUpload = function onUpload(event) {
  ...
  api.upload(data).done(ui.uploadSuccess).fail(ui.uploadFailure);
};

// Read
var onGetFiles = function onGetFiles(event) {
  event.preventDefault();

  api.getFiles().done(ui.getFilesSuccess).fail(ui.getFilesFailure);
};
...
var onEdit = function onEdit(event) {
  ...
  var data = getFormFields(event.target);
  api.edit(id, data).done(ui.editSuccess).fail(ui.editFailure);
};

// Delete
var onDelete = function onDelete(event) {
  event.preventDefault();
  api.deleteFile(event.target.dataset.id).done(ui.deleteSuccess).fail(ui.deleteFailure);
};
...
var loopFiles = function loopFiles(data) {
  $('#myFile-data').empty();
  $('#theirFile-data').empty();
  $('#myFile-data').removeClass('hidden');
  $('#theirFile-data').removeClass('hidden');
  ...
  for (var i = 0; i < files.length; i++) {
    if (files[i]._user === app.user.id) {
      $('#myFile-data').append('<div> ' + '<img src="https://i.imgur.com/mlIU8Y7.png">' + '<h2>' + 'Title:' + files[i].title + '</h2>' + '<p>' + 'Tags:' + files[i].tags + '</p>' + '<button class="delete-file btn btn-default" data-id="' + files[i].id + '" id="' + files[i].id + '" type="button" class="btn">Delete</button>' + '<form class="edit" data-id="' + files[i].id + '">' + '<legend>Edit Tags</legend>' + '<input type="text" name="files[tags]" placeholder="New Tags">' + '<input type="submit" class="edit-file btn btn-default" data-id="' + files[i].id + '" type="button" class="btn"></button>' + '</form>' + '<a class="download-file btn btn-default" href="' + files[i].url + '">Download</a>' + '</div>');
    } else {
      $('#theirFile-data').append('<div> ' + '<img src="https://i.imgur.com/mlIU8Y7.png">' + '<h2>' + 'Title:' + files[i].title + '</h2>' + '<p>' + 'Tags:' + files[i].tags + '</p>' + '<a class="download-file btn btn-default" href="' + files[i].url + '">Download</a>' + '</div>');
    }
  }
};
